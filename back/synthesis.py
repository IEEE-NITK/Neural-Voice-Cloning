...
def get_similarity(Mat1,Mat2):
    total=0
    minLen=min(len(Mat1),len(Mat2))
    minWid = min(len(Mat1[0]),len(Mat2[0]))
    for i in range(minLen):
        data1 = Mat1[i].tolist()
        data2 = Mat2[i].tolist()
        data1 = data1[:minWid]
        data2 = data2[:minWid]
        total+= spatial.distance.cosine(data1,data2)
    return 1-total/minLen
        ...
        for idx, line in enumerate(lines):
            ...
            for i in range(10):
                waveform, alignment, _, mel = tts(
                    model, text, p=replace_pronunciation_prob, speaker_id=i, fast=True)
                dst_wav_path = join(dst_dir, "{}_{}{}{}.wav".format(
                    idx, checkpoint_name, file_name_suffix,i))
                mfcc_features.append(mel)
                dst_alignment_path = join(
                    dst_dir, "{}_{}{}_alignment_for_{}.png".format(idx, checkpoint_name,
                                                            file_name_suffix,i))
...
<div align="center"><img src="/audio/{}/{}/{}" /></div>
    ...
    sim = []
    (rate,sig) = wav.read("audio/in.wav")
    mfcc_feat = mfcc(sig,rate)
    print(mfcc_feat)
    print('---------------')
    print(mfcc_features[0])
    for i in range(len(mfcc_features)):
        sim.append(get_similarity(mfcc_feat,mfcc_features[i]))
    print("The most common ones")
    print(sim)
    print(len(sim))
    print(sim.index(min(sim)))
