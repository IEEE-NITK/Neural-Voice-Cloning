...
class TextDataSource(FileDataSource):
    def __init__(self, data_root, speaker_id=None,train_set=0):
        '''
        train_set : 1 for training set(84 speakers)
                2 for validation set(8 speakers)
                3 for cloning audios set(16 speakers)
                0 for the whole set(108 speakers)
        '''
    ...
    def collect_files(self):
        ...
        with open(meta, "rb") as f:
            lines = f.readlines()
        '''
        checks for if the format of the train txt is fine and splits on |
        to make[['vctk-spec-00002.npy'],
       ['vctk-mel-00002.npy'],
       ['197'],
       ...
       ['0\n']]
        ...
        '''
        In the train.txt the 4th element(index 3) are the text and 5th is the speaker number
        '''
        ...
        if self.multi_speaker:
            ...
            elif self.train_set == 1:#training set
                indices = np.array(speaker_ids) <= 83
                texts = list(np.array(texts)[indices])
                speaker_ids = list(np.array(speaker_ids)[indices])
                self.multi_speaker = False
                return texts

            elif self.train_set == 2:#validation set
                indices = (np.array(speaker_ids) >= 84) & (np.array(speaker_ids) <= 91)
                texts = list(np.array(texts)[indices])
                speaker_ids = list(np.array(speaker_ids)[indices])
                self.multi_speaker = False
                return texts

            elif self.train_set == 3:#cloning audios set
                indices = np.array(speaker_ids) >= 92
                texts = list(np.array(texts)[indices])
                speaker_ids = list(np.array(speaker_ids)[indices])
                self.multi_speaker = False
                return texts
...
class _NPYDataSource(FileDataSource):
    def __init__(self, data_root, col, speaker_id=None,train_set = 0):
        '''
        train : 1 for training set(84 speakers)
                2 for validation set(8 speakers)
                3 for cloning audios set(16 speakers)
                0 for the whole set(108 speakers)
        '''
...
class MelSpecDataSource(_NPYDataSource):
    def __init__(self, data_root, speaker_id=None,train=0):
        super(MelSpecDataSource, self).__init__(data_root, 1, speaker_id,train)


class LinearSpecDataSource(_NPYDataSource):
    def __init__(self, data_root, speaker_id=None,train=0):
        super(LinearSpecDataSource, self).__init__(data_root, 0, speaker_id,train)
...
if __name__ == "__main__":
    ...
    hparams.parse('builder=deepvoice3_multispeaker,preset=deepvoice3_speaker_adaptation_vctk')
